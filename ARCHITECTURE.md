...
| `src/panel/InspectorPanel.tsx` | Shows the selected node's resolved attributes (inherited + own, matching `resolveType`'s merge), plus "+ Add hosted asset" to create a new asset HOSTS-connected to the selected one (e.g. adding a disk to a server). Also a "Groups" section (`GroupMembership`, same file): lists the asset's current `MEMBER_OF` targets and a picker to join another existing `core/group` asset, calling `graph.joinGroup`. Fetches `listAssets({ typeId: "core/group" })` and the asset's own relationships itself on mount rather than threading them through props, since this data isn't otherwise needed by the graph/canvas. The Join button awaits `onJoinGroup` and only updates the local membership list on success, alerting on failure — matching `CreateAssetDialog`'s established convention for a post-creation linking call that can fail after the point of no return; an earlier version updated local state optimistically before the (unawaited) call settled, so a rejected join silently showed as joined. |
